...
int insert_arec(ele in_rec) {   //하나의 레코드를 삽입  key = 회사명  
    ...
    if (!root) {   // root가 NULL이면 btree가 비어있음. 맨 첫 노드를 만들어 여기에 넣는다.
        ...
        root->ptr[0] = root->ptr[1] = root->ptr[2] = NULL;      // p0과 p1에 NULL을 넣는다. 
        root->rec[1] = empty;
        root->fill_cnt = 1;
    ...
    */
    ...
    do {
        ...
        if (curr->fill_cnt < MAXK) {
            ...
            bnode.ptr[0] = NULL/* Fill your code */;
            for (j = 0; j < i; j++) {
                bnode.rec[j] = curr->rec[j]/* Fill your code */;
                bnode.ptr[j + 1] = NULL/* Fill your code */;
            }
            ...
            */
            ...
            new_ptr->fill_cnt = 1;
            for (int index = 0; index < MAXK; index++) {
                new_ptr->ptr[index] = NULL;
                curr->ptr[index] = NULL;
...
void redistribution(nodeptr father, nodeptr l_sibling, nodeptr r_sibling, char wcase, int j) {
    ...
    twoB.rec[i] = father->rec[j]/* Fill your code */; // 부모에서의 중간 키를 가져옴.
    i++;
